...
bl_info = {
    ...
    "version": (2, 0),
    "blender": (2, 78, 0),
    "location": "File > Export > SQEE (.sqm)",
    "description": "Export mesh to an ascii .sqm file",
    "warning": "",
    "wiki_url": "",
    "tracker_url": "",
...
class SqExportMesh_operator(bpy.types.Operator, ExportHelper):
    """Export Mesh to an ascii .sqm file"""
    ...
    exportMode = bpy.props.EnumProperty(
        name = "Format", description = "File Format", default = "ascii",
        items = [("ascii", "ASCII", ""), ("binary", "Binary", "")]
    )
    ...
    def execute(self, context):
        ...
        if self.exportBounds: # bounding box
        ...
        sqm.subMeshList = list(map(SqeeMesh.SubMesh, mesh.materials.keys()))
        if not sqm.subMeshList: sqm.subMeshList = [SqeeMesh.SubMesh("Default")]


        if sqm.hasBone: # sorted list of armature bones
        ...
        ##### Export to ASCII Mesh File #####

        o = open(self.filepath, 'w', encoding='utf-8')
        ...
        if self.exportBounds: # write bounding info
